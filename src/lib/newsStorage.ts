...
function writeNews(news: NewsItem[]): void {
  try {
    ensureDataDir();
...
export function addNews(newsItem: Omit<NewsItem, 'id' | 'dateObtained'>): NewsItem {
  const news = readNews();
...
export function addNewsBulk(newsItems: Omit<NewsItem, 'id' | 'dateObtained'>[]): NewsItem[] {
  const news = readNews();
  const existingUrls = new Set(news.map(item => item.url));
  
  const newItems: NewsItem[] = newsItems
...
export function getFilteredNews(filters: NewsFilters = {}): NewsItem[] {
  let news = readNews();
...
export function getNewsStats(): {
  total: number;
  byCategory: { error: number; corruption: number };
  ...
  const byCategory = news.reduce(
    ...
    { error: 0, corruption: 0 } as { error: number; corruption: number }
  );
